---
name: git-worktrees
description: Manage Git worktrees for parallel Codex development. Use this skill when engineers ask to "create a worktree", "run parallel Codex sessions", "work on multiple features simultaneously", or need help with worktree management.
---
...
Run multiple Codex sessions in parallel on different branches using Git worktrees. This skill provides simple scripts and workflows to set up, manage, and clean up worktrees, enabling true parallel development without conflicts.

**Why Worktrees?**
- **Parallel Development**: Run multiple Codex instances simultaneously
...
- **Long-Running Tasks**: Let Codex work in background while you continue in main
...
- ✅ Give you the Codex setup command
...
**Scenario:** You want Codex to build feature A while you work on feature B.
...
2. **Open Codex in the new worktree:**
   - The script outputs the path
   - Open Codex and navigate to that directory
   - Run `/init` to orient Codex
...
**Scenario:** You want Codex to refactor a large module while you continue development.
...
2. **Start Codex in refactor worktree:**
   ```
...
**Scenario:** You want 3 Codex instances working on different features simultaneously.
...
2. **Open 3 Codex sessions:**
   - Session 1: `../repo-feature-api-endpoints/`
   - Session 2: `../repo-feature-ui-dashboard/`
   - Session 3: `../repo-feature-email-notifications/`

3. **Assign tasks to each Codex:**
   - Codex 1: "Build REST API endpoints for user management"
   - Codex 2: "Create admin dashboard UI"
   - Codex 3: "Implement email notification system"
...
**Scenario:** Production bug needs immediate fix while Codex is working on a feature.

**Steps:**

1. **Codex is already working in a feature worktree**
...
3. **Fix the bug yourself or with another Codex session:**
   ```bash
...
When you open a new Codex session in a worktree, **always run `/init` first**:
...
2. **Open Codex** in worktree directory
3. **Run `/init`** to orient Codex
4. **Give task** to Codex
...
**✅ Use worktrees when:**
- Running multiple Codex sessions in parallel
...
### Issue: Codex seems "lost" in worktree

**Cause:** Didn't run `/init` after opening worktree

**Solution:** Always run `/init` when starting Codex in a new worktree:
```
/init
```
...
Create a `/worktree` command for Codex. See `references/slash_command_template.md` for the complete setup.
...
**Execution:**
- Codex 1 in `../repo-feature-api`: "Build REST API for user management"
- Codex 2 in `../repo-feature-ui`: "Create React UI for user management"
...
**Scenario:** Need to review PR while Codex works on feature

**Setup:**
```bash
# Codex is working in feature worktree
...
- Codex continues work in `../repo-feature-xyz`
...
### 2. **Always Run /init**
   - First thing when opening Codex in a new worktree
...
Git worktrees enable true parallel development with Codex:

✅ **Run multiple Codex sessions** without conflicts
