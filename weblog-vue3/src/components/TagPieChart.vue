<template>
    <el-card shadow="never" class="admin-chart-card admin-filter-card">
        <template #header>
            <div>
                <div class="admin-table-title">标签占比</div>
                <div class="admin-table-desc">各标签下的文章数量分布</div>
            </div>
        </template>
...
import { getTagRatio } from '@/api/admin/dashboard'

const chartRef = ref(null)
let chartInstance = null

const renderChart = async () => {
    const response = await getTagRatio()
    if (!response?.success || !response.data) return

    const rawData = response.data
    const names = rawData.names || rawData.tags || []
