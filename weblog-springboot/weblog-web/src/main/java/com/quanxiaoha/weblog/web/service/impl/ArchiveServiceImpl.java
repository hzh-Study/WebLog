...
public class ArchiveServiceImpl extends ServiceImpl<ArticleMapper, ArticleDO> implements ArchiveService {
    ...
    public Response queryArchive(QueryArchivePageListReqVO queryArchivePageListReqVO) {
        ...
        IPage<ArticleDO> articleDOIPage = articleDao.queryArticlePageList(current, size, null, null);
        ...
        if (!CollectionUtils.isEmpty(records)) {
            itemRspVOList = records.stream()
                    .map(articleDO -> {
                        QueryArchiveItemRspVO item = articleConvert.convert2Archive(articleDO);
                        UserDO author = userMapper.selectById(articleDO.getUserId());
                        if (author != null) {
